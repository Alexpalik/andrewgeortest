...
import Policy from "../product/[slug]/Policy";
import ModalViewAllReviews from "../product/[slug]/ModalViewAllReviews";
        ...
        <div className="flex mt-3">
          {variants.map((variant, index) => (
            <div
              ...
              className={`relative flex-1 max-w-[75px] h-10 sm:h-11 rounded-none border-2 cursor-pointer ${
                variantActive === index
        ...
        <header className="container mt-8 sm:mt-10">
          <div className="relative ">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 lg:gap-6">
              <div
                className="md:h-full col-span-2 md:col-span-1 row-span-2 relative rounded-none sm:rounded-none cursor-pointer"
                onClick={handleOpenModalImageGallery}
              >
                <NcImage
                  alt="firt"
                  containerClassName="aspect-w-3 aspect-h-4 relative md:aspect-none md:absolute md:inset-0"
                  className="object-cover rounded-none sm:rounded-none"
                  src={LIST_IMAGES_GALLERY_DEMO[0]}
                  fill
                  sizes="(max-width: 640px) 100vw, 50vw"
                  priority
                />
                <div className="absolute inset-0 bg-neutral-900/20 opacity-0 hover:opacity-40 transition-opacity rounded-none sm:rounded-none"></div>
              </div>

              {/*  */}
              <div
                className="col-span-1 row-span-2 relative rounded-none sm:rounded-none overflow-hidden z-0 cursor-pointer"
                onClick={handleOpenModalImageGallery}
              >
                <NcImage
                  alt=""
                  ...
                  className="object-cover w-full h-full rounded-none sm:rounded-none"
              ...
              {[LIST_IMAGES_GALLERY_DEMO[2], LIST_IMAGES_GALLERY_DEMO[3]].map(
                (item, index) => (
                  <div
                    key={index}
                    className={`relative rounded-none sm:rounded-none overflow-hidden z-0 ${
                      index >= 2 ? "block" : ""
                    }`}
                  >
                    <NcImage
            ...
            <div
              className="absolute hidden md:flex md:items-center md:justify-center left-3 bottom-3 px-4 py-2 rounded-none bg-white text-slate-500 cursor-pointer hover:bg-slate-200 z-10"
              onClick={handleOpenModalImageGallery}
            >
              <svg
