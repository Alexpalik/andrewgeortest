...
interface Props {
  ...
  productDescription?: string; // ✅ New prop
}
...
const AccordionInfo: FC<Props> = ({
  ...
  const parsedData = data.map((item) => {
    if (item.name === "Description" && productDescription) {
      return {
        ...item,
    ...
    <div className="w-full rounded-2xl space-y-2.5">
      {parsedData.map((item, index) => (
        <Disclosure key={index} defaultOpen={index < 2}>
          {({ open }) => (
            <>
              <Disclosure.Button className="flex items-center justify-between w-full px-4 py-2 font-medium text-left bg-slate-100/80 hover:bg-slate-200/60 dark:bg-slate-800 dark:hover:bg-slate-700 focus:outline-none focus-visible:ring focus-visible:ring-slate-500 focus-visible:ring-opacity-75">
                <span>{item.name}</span>
                {!open ? (
